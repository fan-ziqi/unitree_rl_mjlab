...
def unitree_go2w_spin_stance_ppo_runner_cfg() -> RslRlOnPolicyRunnerCfg:
  return _trick_runner_cfg(
    ...
    entropy_coef=0.005,
    # The fixed-command audits show an abrupt left-side mode collapse between
    # adjacent checkpoints even though the right mirror remains good.  A more
    # conservative PPO step reduces this shared-policy overwrite; it changes
    # no observation, action, or reward semantics.
...
def unitree_go2w_aerial_rotation_ppo_runner_cfg() -> RslRlOnPolicyRunnerCfg:
  return _trick_runner_cfg(
    ...
    # Equal five-way sampling keeps each signed axis in the batch; a smaller
    # update prevents a newly successful branch from overwriting its mirrors.
    learning_rate=2.5e-4,
  )
