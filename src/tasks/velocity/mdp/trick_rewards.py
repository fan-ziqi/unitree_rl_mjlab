...
def aerial_axis_spin_rate(
  env: ManagerBasedRlEnv,
  command_name: str,
  sensor_name: str,
  nonwheel_sensor_name: str,
  target_angle: float,
  target_axis_speed: float,
  target_clearance: float,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Reward sustained *signed* angular speed during the first legal flight.

  Net angle is the task result and remains the dominant aerial reward.  It is
  nevertheless a delayed measurement: a policy which produces a brief angular
  spike and then sheds that angular momentum receives nearly the same first
  gradient as one that carries the body through the whole ballistic arc.  This
  term supplies that missing physical predecessor.  It has no pose, action,
  phase, or reference trajectory: it only measures positive rotation about
  the one-hot's already-defined world axis while the wheels are actually off
  the floor.  It is disabled after one full turn, so spinning faster forever
  cannot be an alternative objective to a quiet landing.
  """
  if target_angle <= 0.0 or target_axis_speed <= 0.0 or target_clearance <= 0.0:
    raise ValueError("aerial axis-spin targets must be positive.")
  command = _command(env, command_name)
  active = torch.sum(command[:, :5], dim=1) > 0.5
  command_term = env.command_manager.get_term(command_name)
  qualified_flight = getattr(
    command_term,
    "_current_flight_qualified",
  ...
  airborne = ~_has_any_contact(env, sensor_name)
  legal = ~_has_any_contact(env, nonwheel_sensor_name)
  asset: Entity = env.scene[asset_cfg.name]
  default_root_state = asset.data.default_root_state
  assert default_root_state is not None
  ...
  rate_score = torch.clamp(signed_axis_rate / target_axis_speed, min=0.0, max=1.0)
  under_target = progress < target_angle
  return (
    active.to(rate_score.dtype)
    * qualified_flight.to(rate_score.dtype)
    * airborne.to(rate_score.dtype)
    * legal.to(rate_score.dtype)
    * under_target.to(rate_score.dtype)
